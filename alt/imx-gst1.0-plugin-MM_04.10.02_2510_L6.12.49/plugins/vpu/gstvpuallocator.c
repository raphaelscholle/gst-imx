...
gst_vpu_alloc_phys_mem(G_GNUC_UNUSED GstAllocatorPhyMem *allocator, PhyMemBlock *memory)
{
	...
	GST_DEBUG_OBJECT(allocator, "vpu allocator malloc size: %" G_GSIZE_FORMAT "\n", memory->size);
...
gst_vpu_free_phys_mem(G_GNUC_UNUSED GstAllocatorPhyMem *allocator, PhyMemBlock *memory)
{
  ...
  VpuMemDesc mem_desc;
	...
	GST_DEBUG_OBJECT(allocator, "vpu allocator free size: %" G_GSIZE_FORMAT "\n", memory->size);
  memset(&mem_desc, 0, sizeof(VpuMemDesc));
    ...
    PhyMemBlock *src_mem, guint offset, guint size)
{
  ...
  GST_DEBUG_OBJECT(allocator, "vpu allocator copy size: %" G_GSIZE_FORMAT "\n", src_mem->size);
  memset(&mem_desc, 0, sizeof(VpuMemDesc));
